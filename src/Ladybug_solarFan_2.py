...
class CleanBrep:
    ...
    def unionFans(self): 
        ...
        if len(fanlst) != 1: # safeUnionMethod has failed
...
def cleanCurve(boundary):
    # simplify, reverse curve direction
    rs.SimplifyCurve(boundary)
    b_ = rs.coercecurve(boundary)
    Rhino.Geometry.Curve.Reverse(b_)
    sc.doc.Objects.Replace(boundary,b_)
    return boundary
...
def getSolarGeom(boundary):
    ...
    top_plane = get_plane(boundary,height)

    # project curve to user_defined height
    sun_pts = get_sunpt(latitude,centerPt,6,day,shourlst,north_=north,lon=longitude,tZ=timeZone,scale_=100)
    sun_pts.extend(get_sunpt(latitude,centerPt,9,day,ehourlst,north_=north,lon=longitude,tZ=timeZone,scale_=100))
